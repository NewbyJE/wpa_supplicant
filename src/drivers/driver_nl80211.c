...
static int android_priv_cmd(struct i802_bss *bss, const char *cmd)
{
	...
	wpa_printf(MSG_ERROR, "%s: issuing %s private command\n", __func__, cmd);
			     ...
			     struct wpa_driver_scan_params *params)
{
	...
	wpa_printf(MSG_ERROR, "%s: issuing pnosetup private command\n", __func__);
	ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 1, &ifr);
...
static int wpa_driver_nl80211_driver_cmd(void *priv, char *cmd, char *buf, size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct ifreq ifr;
	...
	if (os_strcasecmp(cmd, "STOP") == 0) {
		...
		wpa_printf(MSG_ERROR, "%s: failed to issue private commands (CMD=%s)\n", __func__, cmd);
			...
			if ((os_strcasecmp(cmd, "LINKSPEED") == 0) ||
				...
				wpa_printf(MSG_DEBUG, "%s: P2P: Device address ("MACSTR")",	__func__, MAC2STR(buf));
